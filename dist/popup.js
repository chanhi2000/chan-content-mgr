...
const btnAdrianRoselliBlog = document.getElementById('fetch-adrianroselli-blog');
const btnCssTipBlog = document.getElementById('fetch-css-tip');
const btnAListApartBlog = document.getElementById('fetch-alistapart');
const btn9ElementsBlog = document.getElementById('fetch-9-elements');
const btnAdamArgyleBlog = document.getElementById('fetch-adam-argyle');
const btnTobiasAhlinBlog = document.getElementById('fetch-tobias-ahlin');
const btnCssIrlBlog = document.getElementById('fetch-css-irl-blog');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  getCurrentTab().then((tab) => {
    ...
    if (/freecodecamp\.org\/news/g.test(tab.url)) {
      ...
      btnSmashingMagazineBlog.disabled = false
      btnSmashingMagazineBlog.style.display = 'block';
      labelArticlePath.value = tab.url
      ...
      labelArticlePath.value = tab.url
        .replace(/(https:\/\/)|(www\.)|(bram\.us\/)|(\d{4}\/\d{2}\/\d{2}\/)/g, '')
    } else if (/joshwcomeau\.com\//g.test(tab.url)) {
      detailsBlog.disabled = false;
      detailsBlog.open = true;
      detailsBlog.style.background = 'rgba(128,159,255,0.2)'
      summaryBlog.classList.add('activated')
      summaryBlog.innerHTML = `${makeIcon("https://joshwcomeau.com/favicon.png")}<span>joshwcomeau.com</span>`;
      btnJoshWComeauBlog.disabled = false
    ...
    } else if (/adrianroselli\.com\//g.test(tab.url)) {
      detailsBlog.disabled = false;
      detailsBlog.open = true;
      detailsBlog.style.background = 'rgba(0,0,0,0.2)'
      summaryBlog.classList.add('activated')
      summaryBlog.innerHTML = `${makeIcon("https://adrianroselli.com/wp-content/themes/AAR/favicon.png")}<span>adrianroselli.com</span>`;
      btnAdrianRoselliBlog.disabled = false
      btnAdrianRoselliBlog.style.display = 'block';
      labelArticlePath.value = tab.url
        .replace(/(https:\/\/)|(www\.)|(adrianroselli\.com\/)/g, '')
    } else if (/nerdy\.dev\//g.test(tab.url)) {
      detailsBlog.disabled = false;
      detailsBlog.open = true;
      detailsBlog.style.background = 'rgba(137,41,255,0.2)'
      summaryBlog.classList.add('activated')
      summaryBlog.innerHTML = `${makeIcon("https://nerdy.dev/favicon.svg")}<span>nery.dev</span>`;
      btnAdamArgyleBlog.disabled = false
      btnAdamArgyleBlog.style.display = 'block';
      labelArticlePath.value = tab.url
        .replace(/(https:\/\/)|(www\.)|(nerdy\.dev\/)/g, '')
    } else if (/tobiasahlin\.com\//g.test(tab.url)) {
      detailsBlog.disabled = false;
      detailsBlog.open = true;
      detailsBlog.style.background = 'rgba(43,47,60,0.2)'
      summaryBlog.classList.add('activated')
      summaryBlog.innerHTML = `${makeIcon("https://tobiasahlin.com/images/touch-icon-ipad-retina.png")}<span>tobiasahlin.com</span>`;
      btnTobiasAhlinBlog.disabled = false
      btnTobiasAhlinBlog.style.display = 'block';
      labelArticlePath.value = tab.url
        .replace(/(https:\/\/)|(www\.)|(tobiasahlin\.com\/)/g, '')
    } else if (/css-irl\.info\//g.test(tab.url)) {
      detailsBlog.disabled = false;
      detailsBlog.open = true;
      detailsBlog.style.background = 'rgba(255,20,147,0.2)'
      summaryBlog.classList.add('activated')
      summaryBlog.innerHTML = `${makeIcon("https://css-irl.info/favicon/apple-touch-icon.png")}<span>css-irl.info</span>`;
      btnCssIrlBlog.disabled = false
      btnCssIrlBlog.style.display = 'block';
      labelArticlePath.value = tab.url
        .replace(/(https:\/\/)|(www\.)|(css-irl\.info\/)/g, '')
    } else if (/dbushell\.com\//g.test(tab.url)) {
      detailsBlog.disabled = false;
      detailsBlog.open = true;
      detailsBlog.style.background = 'rgba(0,150,190,0.2)'
      summaryBlog.classList.add('activated')
      summaryBlog.innerHTML = `${makeIcon("https://dbushell.com/assets/icons/favicon.svg")}<span>dbushell.com</span>`;
      btnDavidBushellBlog.disabled = false
      btnDavidBushellBlog.style.display = 'block';
      labelArticlePath.value = tab.url
        .replace(/(https:\/\/)|(www\.)|(dbushell\.com\/)|(\d{4}\/\d{2}\/\d{2}\/)/g, '')
    } else if (/webdevredfox\.org\//g.test(tab.url)) {
      detailsBlog.disabled = false;
      detailsBlog.open = true;
      detailsBlog.style.background = 'rgba(255,127,80,0.2)'
      summaryBlog.classList.add('activated')
      summaryBlog.innerHTML = `${makeIcon("https://svelte.dev/favicon.png")}<span>webdevredfox.org</span>`;
      btnWebDevRedFoxBlog.disabled = false
      btnWebDevRedFoxBlog.style.display = 'block';
      labelArticlePath.value = tab.url
        .replace(/(https:\/\/)|(www\.)|(webdevredfox\.org\/)/g, '')
    } else if (/alwaystwisted\.com\//g.test(tab.url)) {
      detailsBlog.disabled = false;
      detailsBlog.open = true;
      detailsBlog.style.background = 'rgba(255,117,0,0.2)'
      summaryBlog.classList.add('activated')
      summaryBlog.innerHTML = `${makeIcon("https://alwaystwisted.com/images/favicons/apple-touch-icon.png")}<span>alwaystwisted.com</span>`;
      btnAlwaysTwistedBlog.disabled = false
    ...
    } else if (/web\.dev\//g.test(tab.url)) {
      detailsBlog.disabled = false;
      detailsBlog.open = true;
      detailsBlog.style.background = 'rgba(26,115,232,0.2)'
      summaryBlog.classList.add('activated')
      summaryBlog.innerHTML = `${makeIcon("https://gstatic.com/devrel-devsite/prod/v579073a50c63499824df5a68b8922367066583d283ef78fdade1028efdb4ceb5/web/images/touchicon-180.png")}<span>web.dev</span>`;
      btnWebDevBlog.disabled = false
      btnWebDevBlog.style.display = 'block';
      labelArticlePath.value = tab.url
        .replace(/(https:\/\/)|(www\.)|(web\.dev\/)|(articles\/)/g, '')
    ...
    } else if (/alistapart\.com\//g.test(tab.url)) {
      detailsBlog.disabled = false;
      detailsBlog.open = true;
      detailsBlog.style.background = 'rgba(34,34,34,0.2)'
      summaryBlog.classList.add('activated')
      summaryBlog.innerHTML = `${makeIcon("https://i0.wp.com/alistapart.com/wp-content/uploads/2019/03/cropped-icon_navigation-laurel-512.jpg?fit=192%2C192&ssl=1")}<span>alistapart.com</span>`;
      btnAListApartBlog.disabled = false
      btnAListApartBlog.style.display = 'block';
      labelArticlePath.value = tab.url
        .replace(/(https:\/\/)|(www\.)|(alistapart\.com\/)/g, '')
    } else if (/9elements\.com\//g.test(tab.url)) {
      detailsBlog.disabled = false;
      detailsBlog.open = true;
      detailsBlog.style.background = 'rgba(0,5,4,0.2)'
      summaryBlog.classList.add('activated')
      summaryBlog.innerHTML = `${makeIcon("https://9elements.com/assets/images/meta/favicon.svg")}<span>9elements.com</span>`;
      btn9ElementsBlog.disabled = false
      btn9ElementsBlog.style.display = 'block';
      labelArticlePath.value = tab.url
    ...
    } else if (/toss\.tech/g.test(tab.url)) {
      detailsBlog.disabled = false;
      detailsBlog.open = true;
      detailsBlog.style.background = 'rgba(34,114,235,0.2)'
      summaryBlog.classList.add('activated')
      summaryBlog.innerHTML = `${makeIcon("https://static.toss.im/tds/favicon/favicon.ico")}<span>toss.tech</span>`;
      btnTossTechBlog.disabled = false
      btnTossTechBlog.style.display = 'block';
      labelArticlePath.value = tab.url.replace(/(https:\/\/)|(www\.)|(toss\.tech\/)|(articles\/)/g, '');
    } else {
...
btnAdrianRoselliBlog.addEventListener('click', async () =>     await onClickWebScraping('FETCH_ADRIAN_ROSELLI_BLOG'));
...
btnAListApartBlog.addEventListener('click', async () =>        await onClickWebScraping('FETCH_ALISTAPART_BLOG'));
btn9ElementsBlog.addEventListener('click', async () =>         await onClickWebScraping('FETCH_9_ELEMENTS_BLOG'));
btnAdamArgyleBlog.addEventListener('click', async () =>        await onClickWebScraping('FETCH_ADAM_ARGYLE_BLOG'));
btnCssIrlBlog.addEventListener('click', async() =>             await onClickWebScraping('FETCH_CSS_IRL_BLOG'));
...
btnWebDevBlog.addEventListener('click', async() =>             await onClickWebScraping('FETCH_WEB_DEV_BLOG'));
btnTossTechBlog.addEventListener('click', async () =>          await onClickWebScraping('FETCH_TOSS_TECH'));
